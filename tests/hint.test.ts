...
describe('own-view move hints',()=>{
  ...
  it('explains setting the bid from the defender perspective and names ties honestly',async()=>{
    const g=position(true),reply=estimate(g,350);vi.mocked(api).mockResolvedValue(reply);
    const hint=await getHint(g,'hint-test',350);
    expect(hint.stats!.options[0]!.chance).toBe(1);
    expect(hintExplanation(hint)).toContain('Your team set the 30 bid in 350 of 350 sampled deals');
    reply.response.evaluation!.options=reply.response.evaluation!.options.map(([t])=>[t,'1','2']);
    const tied=await getHint(g,'hint-test',350);
